...
function fallbackFindings(title: string, sourceText: string): FindingsPayload {
...
async function openAiFindings(title: string, sourceText: string): Promise<FindingsPayload> {
  ...
  if (!response.ok) {
    throw new Error(
      payload?.error?.message ?? "OpenAI findings generation failed."
    );
  }
  ...
  return {
    summaryShort:
      typeof parsed.summaryShort === "string"
        ? parsed.summaryShort
        : fallbackFindings(title, sourceText).summaryShort,
    summaryFull:
      typeof parsed.summaryFull === "string"
        ? parsed.summaryFull
        : fallbackFindings(title, sourceText).summaryFull,
...
export async function generateMeetingFindings(title: string, sourceText: string) {
  ...
  try {
    ...
    console.warn("[workspace] Falling back from OpenAI findings generation", error);
    return fallbackFindings(title, normalizedText);
  }
}
