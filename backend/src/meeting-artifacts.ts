...
export function buildCanonicalArtifact(args: {
  ...
  } satisfies CanonicalMeetingArtifact;
}
...
export function parseCanonicalArtifact(artifact: MeetingArtifactRecord | null | undefined) {
...
export function deriveStructuredMeetingContent(args: {
  ...
  return {
    ...
    decisions: compact(args.findings?.decisions_json ?? [], "No decisions captured."),
    actionItems: compact(args.findings?.action_items_json ?? [], "No action items captured."),
